...
`个人仓库 <https://github.com/erbiaoger/code/tree/main/Fortran>`__
==================================================================
...
| 实型                  | real :: a             | 存储的浮点数          |
+-----------------------+-----------------------+-----------------------+
| 复数                  | complex, parameter :: | 一                    |
|                       | b = (10, 2)           | 个复杂的数字有两部分  |
|                       |                       | ，实数部分和虚数部分  |
+-----------------------+-----------------------+-----------------------+
| 布尔                  | logical, parameter :: | 只                    |
|                       | c = .true.            | 有两个逻辑值：.true.  |
|                       |                       | 和.false. 关系运算符  |
|                       |                       | .and. .or. .not.      |
|                       |                       | .eqv. .neqv.          |
+-----------------------+-----------------------+-----------------------+
| 字符                  | character(len=40),    | 字符串的长度可以      |
|                       | name                  | 通过len个符来指定。如 |
|                       |                       | 果没有指定长度，它是1 |
+-----------------------+-----------------------+-----------------------+
...
|                                   | （如果有的话）被执行，依此类推。  |
+-----------------------------------+-----------------------------------+
| 内嵌 if 结构                      | 可以使用一个 **if** 或 **else     |
|                                   | if*                               |
|                                   | * 语句在另外一个 **if** 或 **else |
|                                   | if** 语句内部                     |
+-----------------------------------+-----------------------------------+
| select case 语句                  | Select                            |
|                                   | Case语句允许一个                  |
|                                   | 变量的值对的列表，平等进行测试。  |
+-----------------------------------+-----------------------------------+
| 内嵌select case 结构              | 可以使用一个SELECT                |
|                                   | CASE语句中的另一个选择case语句。  |
+-----------------------------------+-----------------------------------+
