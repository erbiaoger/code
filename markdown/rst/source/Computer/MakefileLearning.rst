...
。如果当前目录中，source.txt
已经存在，那么\ ``make result.txt``\ 可以正常运行，否则必须再写一条规则，来生成
source.txt 。
...
Make命令提供一系列内置变量，比如，\ :math:`(CC) 指向当前使用的编译器，`\ (MAKE)
指向当前使用的Make工具。这主要是为了跨平台的兼容性
...
:math:`? 指代比目标更新的所有前置条件，之间以空格分隔。比如，规则为 t: p1 p2，其中 p2 的时间戳比 t 新，`?就指代p2。
...
$(@D) 和
:math:`(@F) 分别指向 `\ @ 的目录名和文件名。比如，\ :math:`@是 src/input.c，那么`\ (@D)
的值为 src ，$(@F) 的值为 input.c。
